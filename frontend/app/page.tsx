...
// [tiêu đề, mô tả, link (nếu đã có trang)]
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex gap-3">
        <Link href="/extract" className="rounded bg-indigo-600 px-4 py-2 text-white">
          Trích xuất đề án (AI) →
        </Link>
        <Link href="/programs" className="rounded border border-indigo-600 px-4 py-2 text-indigo-700">
          Danh sách CTĐT
        </Link>
      </div>
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {MODULES.map(([title, desc, href]) => {
          const card = (
            <div
              className={`h-full rounded-lg border bg-white p-4 shadow-sm ${
                href ? "transition hover:border-indigo-400 hover:shadow" : ""
              }`}
            >
              <h3 className="font-semibold text-indigo-700">
                {title} {href && <span className="text-xs text-indigo-400">→</span>}
              </h3>
